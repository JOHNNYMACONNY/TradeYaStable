...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    // UserChallenges (top-level)
    // Document ID pattern: userId_challengeId
    match /userChallenges/{userChallengeId} {
      function isOwnerByCompositeId(id) {
        return id.split('_')[0] == request.auth.uid;
      }
      allow read: if isAuthenticated() && (isOwnerByCompositeId(userChallengeId) || isAdmin());
      allow create: if isAuthenticated() && isOwnerByCompositeId(userChallengeId);
      allow update: if isAuthenticated() && (isOwnerByCompositeId(userChallengeId) || isAdmin());
      allow delete: if isAdmin();
    }

    // ChallengeSubmissions (top-level)
    match /challengeSubmissions/{submissionId} {
      // Public read allowed when isPublic == true; otherwise owner or admin
      allow read: if (
